...
class EditComponent extends React.Component{
    ...
    .then(res=>{
      if (res.data.success) {
        ...
        this.setState({
          ...
          selectFuncao:data.funcaoId,
          stringFuncao:data.funcao.funcao          
  ...
  render(){
    return (
      <div>
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="inputState">Funcao</label>
            <select id="inputState" class="form-control" onChange={(value) => this.setState({selectFuncao:value.target.value})}>
              <option selected value = {this.state.dataEmployee.funcaoId}>{this.state.stringFuncao}</option>
  ...
  sendUpdate(){
    ...
    const datapost = {
      ...
      funcao: this.state.selectFuncao,
    }
