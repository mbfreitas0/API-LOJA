    ...
    mysql.getConnection((error, conn) =>{
        ...
        conn.query(
           'INSERT INTO produtos(id_grupo, id_marca, id_locacao, nome, preco, custo, qtd_estoque, imagem_produto) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [req.body.id_grupo, req.body.id_marca, req.body.id_locacao, req.body.nome, req.body.preco, req.body.custo, req.body.qtd_estoque, req.body.imagem_produto],
            (error, result, field) => {
                conn.release();
                if(error){return res.status(500).send({ error : error })}
                const response = {
...
exports.updateProduto = (req, res, next) =>{
    mysql.getConnection((error, conn) =>{
        if(error){return res.status(500).send({ error : error })}
        conn.query(
           'UPDATE produtos SET id_grupo = ?, id_marca = ?, id_locacao = ?, nome = ?, preco = ?, custo = ?, qtd_estoque = ? imagem_produto = ? WHERE id_produto = ?',
           [req.body.id_grupo, req.body.id_marca, req.body.id_locacao, req.body.nome, req.body.preco, req.body.custo, req.body.qtd_estoque, req.body.imagem_produto],
            (error, result, field) => {
                conn.release();
                if(error){return res.status(500).send({ error : error })}
