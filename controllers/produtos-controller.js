...
exports.postProduto = (req, res, next) =>{
    ...
    mysql.getConnection((error, conn) =>{
        if(error){return res.status(500).send({ error : error })}
        conn.query(
           'INSERT INTO produtos(id_grupo, id_marca, id_locacao, descricao, estoque_max, estoque_min, qtd_estoque, imagem_produto) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
exports.updateProduto = (req, res, next) =>{
    mysql.getConnection((error, conn) =>{
        if(error){return res.status(500).send({ error : error })}
        conn.query(
           'UPDATE produtos SET id_grupo = ?, id_marca = ?, id_locacao = ?, descricao = ?, estoque_max = ?, estoque_min = ?, qtd_estoque = ? imagem_produto = ? WHERE id = ?',
           [req.body.id_grupo, req.body.id_marca, req.body.id_locacao, req.body.descricao, req.body.estoque_max, req.body.estoque_min, req.body.qtd_estoque, req.body.imagem_produto],
            (error, result, field) => {
                conn.release();
                if(error){return res.status(500).send({ error : error })}
